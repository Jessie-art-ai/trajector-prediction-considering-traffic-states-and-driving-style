...
def compute_kde_nll(agent_traj, gt_traj):
    kde_ll = 0.
    log_pdf_lower_bound = -20
    num_timesteps = gt_traj[0].shape[0] # 12 ,  gt_traj is list of length= no of agents, list has (12,2) gt
    num_batches = len(agent_traj)  # 269,  agent_traj is list of length = no of agents, list has (20,12,2) predicted samples

    for batch_num in range(num_batches):
        for timestep in range(num_timesteps):
            try:
                kde = gaussian_kde(agent_traj[batch_num][ :, timestep].T)
...
if __name__ == '__main__':
    ...
    for seq_name in seq_eval:
        ...
        for data_file in data_filelist:  # each example e.g., seq_0001 - frame_000009
            ...
            # gt_traj size: [agents, 12, 2]
            # agent_traj size: [agents, 20, 12, 2]
            kde_nll.append(compute_kde_nll(agent_traj, gt_traj))
    ...
    print("MEAN KDE NLL: ", np.mean(kde_nll))  # kde nll
