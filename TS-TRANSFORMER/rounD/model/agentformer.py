...
class PositionalAgentEncoding(nn.Module):
    ...
    def build_pos_enc(self, max_len):
        pe = torch.zeros(max_len, self.d_model) # dim: [200, 256]
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, self.d_model, 2).float() * (-np.log(10000.0) / self.d_model))
        pe[:, 0::2] = torch.sin(position * div_term) # choose every even column
        pe[:, 1::2] = torch.cos(position * div_term) # choose every odd column
        pe = pe.unsqueeze(0).transpose(0, 1) # dim: [200, 1, 256]
        return pe
    ...
    def get_pos_enc(self, num_t, num_a, t_offset):
        pe = self.pe[t_offset: num_t + t_offset, :] # dim: [8, 1, 256]
    ...
    def forward(self, x, num_a, agent_enc_shuffle=None, t_offset=0, a_offset=0):
        num_t = x.shape[0] // num_a  # 8 // num_agents
        pos_enc = self.get_pos_enc(num_t, num_a, t_offset) # dim: [num_t, 1, 256]
        ...
        if self.concat:
            ...
            x = torch.cat(feat, dim=-1) # dim: [8, 1, 512]
            x = self.fc(x) # dim: [8, 1, 256]
        else:
            x += pos_enc
            if self.use_agent_enc:
                x += agent_enc
    ...
    def forward(self, data):
        ...
        for key in self.input_type:
            if key == 'pos':
                ...
                vel = data['pre_vel']  # dim: [7, agents, 2]
                if len(self.input_type) > 1:
                    vel = torch.cat([vel[[0]], vel], dim=0)  # dim: [8, agents, 2]
                if self.vel_heading:
                ...
                traj_in.append(data['pre_motion_scene_norm']) # dim: [8, 1, 2]
            elif key == 'heading':
        ...
        traj_in = torch.cat(traj_in, dim=-1) # dim: [8, 1, 4] # similar to trajectron++, we keep pos, velocity for every agent in single list
        tf_in = self.input_fc(traj_in.view(-1, traj_in.shape[-1])).view(-1, 1, self.model_dim) # dim: [8, 1, 256]
        agent_enc_shuffle = data['agent_enc_shuffle'] if self.agent_enc_shuffle else None
        tf_in_pos = self.pos_encoder(tf_in, num_a=data['agent_num'], agent_enc_shuffle=agent_enc_shuffle) # dim: [8, 1, 256]

        src_agent_mask = data['agent_mask'].clone()  # dim: [agents, agents]
        src_mask = generate_mask(tf_in.shape[0], tf_in.shape[0], data['agent_num'], src_agent_mask).to(tf_in.device) # dim: [8, 8]

        ### added  state variable to tf_in_pos ###
        seq_name = data['seq']
        state_name = seq_name.split("_")[1]
        if state_name == "clump":
            state_variable = torch.Tensor([1, 0, 0, 0, 0, 0, 0, 0]) # added extra 0's to make dim 264 for AgentAwareAttention(embed_dim // n_head)
        elif state_name == "unclump":
            state_variable = torch.Tensor([0, 1, 0, 0, 0, 0, 0, 0])
        elif state_name == "neutral":
            state_variable = torch.Tensor([0, 0, 1, 0, 0, 0, 0, 0])


        state_variable = state_variable.repeat(tf_in_pos.size()[0], tf_in_pos.size()[1], 1).type(torch.float).cuda()
        tf_in_pos = torch.cat([tf_in_pos, state_variable], axis=-1)  # dim: [8, agents, 256 + 8]
        ### added  state variable to tf_in_pos ###

        # added self.linear to reduce dims from 264 to 256 # dim: [8, 1, 256]
        # concatenated with state variable
        data['context_enc'] = torch.cat([self.linear(self.tf_encoder(tf_in_pos, mask=src_mask, num_agent=data['agent_num'])) , state_variable], axis=-1)

        context_rs = data['context_enc'].view(-1, data['agent_num'], self.model_dim + 8) # dim: [8, 1, 256 + 8] # added

        # compute per agent context, each agent will have on feature meaned or maxed over past history timestep
        if self.pooling == 'mean':
            data['agent_context'] = torch.mean(context_rs, dim=0) # dim: [num_agents, 256 + 8]
        else:
...
class FutureEncoder(nn.Module):
    def __init__(self, cfg, ctx, **kwargs):
        ...
        if self.out_mlp_dim is None:
            self.q_z_net = nn.Linear(self.model_dim + 8, num_dist_params) # added
        else:
    ...
    def forward(self, data, reparam=True):
        ...
        for key in self.input_type:
            if key == 'pos':
                ...
                raise ValueError('unknown input_type!')
        traj_in = torch.cat(traj_in, dim=-1) # dim: [12, 1, 4], similar to trajectron++ we concat pos, vel linearly
        tf_in = self.input_fc(traj_in.view(-1, traj_in.shape[-1])).view(-1, 1, self.model_dim)
        agent_enc_shuffle = data['agent_enc_shuffle'] if self.agent_enc_shuffle else None
        tf_in_pos = self.pos_encoder(tf_in, num_a=data['agent_num'], agent_enc_shuffle=agent_enc_shuffle) # dim: [12, 1, 256]

        mem_agent_mask = data['agent_mask'].clone() # dim: [agents, agents]
        tgt_agent_mask = data['agent_mask'].clone()

        mem_mask = generate_mask(tf_in.shape[0], data['context_enc'].shape[0], data['agent_num'], mem_agent_mask).to(tf_in.device) #dim: [12, 8]
        tgt_mask = generate_mask(tf_in.shape[0], tf_in.shape[0], data['agent_num'], tgt_agent_mask).to(tf_in.device) # dim: [12, 12]

        ### added  state variable to tf_in_pos ###
        seq_name = data['seq']
        state_name = seq_name.split("_")[1]
        if state_name == "clump":
            state_variable = torch.Tensor([1, 0, 0, 0, 0, 0, 0,
                                           0])  # added extra 0's to make dim 264 for AgentAwareAttention(embed_dim // n_head)
        elif state_name == "unclump":
            state_variable = torch.Tensor([0, 1, 0, 0, 0, 0, 0, 0])
        elif state_name == "neutral":
            state_variable = torch.Tensor([0, 0, 1, 0, 0, 0, 0, 0])
        ...
        # dim: [12, 1, 256 + 8]
        tf_out = tf_out.view(traj_in.shape[0], -1, self.model_dim + 8) # added

        if self.pooling == 'mean':
            h = torch.mean(tf_out, dim=0) #dim: [1, 256 + 8]--> [agents, 256 + 8]
        else:
            h = torch.max(tf_out, dim=0)[0]

        if self.out_mlp_dim is not None:
            h = self.out_mlp(h)  # dim: [1, 256]

        q_z_params = self.q_z_net(h) # dim: [1, 64]
        ...
        data['q_z_samp'] = data['q_z_dist'].rsample() # dim: [1, 32]
...
class FutureDecoder(nn.Module):
    def __init__(self, cfg, ctx, **kwargs):
        ...
        self.pos_encoder = PositionalAgentEncoding(self.model_dim, self.dropout, concat=ctx['pos_concat'], max_a_len=ctx['max_agent_len'], use_agent_enc=ctx['use_agent_enc'], agent_enc_learn=ctx['agent_enc_learn'])
        if self.out_mlp_dim is None:
            self.out_fc = nn.Linear(self.model_dim + 8, forecast_dim) # added dims
        else:
        ...
        if self.learn_prior:
            num_dist_params = 2 * self.nz if self.z_type == 'gaussian' else self.nz     # either gaussian or discrete
            self.p_z_net = nn.Linear(self.model_dim + 8, num_dist_params) # added
    ...
    def decode_traj_ar(self, data, mode, context, pre_motion, pre_vel, pre_motion_scene_norm, z, sample_num, need_weights=False):
        ...
        # ---> initial sequence F is positions of agents at 8th timestep
        dec_in = dec_in.view(-1, sample_num, dec_in.shape[-1]) # dim: [1, 1, 2] ---> [timestep, agents, 2]
        z_in = z.view(-1, sample_num, z.shape[-1]) # dim: [1, 1, 32]
        in_arr = [dec_in, z_in]
        ...
        dec_in_z = torch.cat(in_arr, dim=-1) # dim: [1, 1, 34] , concat y + z as mentioned in paper

        mem_agent_mask = data['agent_mask'].clone()
        tgt_agent_mask = data['agent_mask'].clone()

        for i in range(self.future_frames):
            tf_in = self.input_fc(dec_in_z.view(-1, dec_in_z.shape[-1])).view(dec_in_z.shape[0], -1, self.model_dim) # dim: [1, 1, 256]
            agent_enc_shuffle = data['agent_enc_shuffle'] if self.agent_enc_shuffle else None
            ...
            # tf_in_pos dim: [1, 1, 256]
            # tf_in_pos = tf_in
            mem_mask = generate_mask(tf_in.shape[0], context.shape[0], data['agent_num'], mem_agent_mask).to(tf_in.device) # dim:[1, 8]
            tgt_mask = generate_ar_mask(tf_in_pos.shape[0], agent_num, tgt_agent_mask).to(tf_in.device) # dim: [1, 1]
            ...
            # tf_out dim: [1, 1, 256 + 8]
            out_tmp = tf_out.view(-1, tf_out.shape[-1]) # dim: [1, 256 + 8]
            if self.out_mlp_dim is not None:
                cat_arr = [out_tmp]
                if 'map' in self.out_cat:
                ...
                out_tmp = torch.cat(cat_arr, dim=-1) # dim: [1, 256]
                out_tmp = self.out_mlp(out_tmp) # dim: [1, 256]
            seq_out = self.out_fc(out_tmp).view(tf_out.shape[0], -1, self.forecast_dim) # dim: [1, 1, 2] --> [time, agents, 2]
    ...
    def forward(self, data, mode, sample_num=1, autoregress=True, z=None, need_weights=False):
        context = data['context_enc'].repeat_interleave(sample_num, dim=1) # dim:[8, agents, 256]      # 80 x 64
        pre_motion = data['pre_motion'].repeat_interleave(sample_num, dim=1) # dim: [8, 1, 2]            # 10 x 80 x 2
        pre_vel = data['pre_vel'].repeat_interleave(sample_num, dim=1) if self.pred_type == 'vel' else None
        pre_motion_scene_norm = data['pre_motion_scene_norm'].repeat_interleave(sample_num, dim=1) # dim: [8, 1, 2]

        # CVAE PRIOR IS CALCULATED HERE
        # p(z)
        prior_key = 'p_z_dist' + ('_infer' if mode == 'infer' else '')
        if self.learn_prior:
            h = data['agent_context'].repeat_interleave(sample_num, dim=0) # dim: [agents, 256]
            p_z_params = self.p_z_net(h) #dim: [1, 64] ---> [agents, 64]
            if self.z_type == 'gaussian':
...
class AgentFormer(nn.Module):
    ...
    def set_data(self, data):
        ...
        self.data['pre_motion'] = torch.stack(in_data['pre_motion_3D'], dim=0).to(device).transpose(0, 1).contiguous() # dim:[8, agents, 2]
        self.data['fut_motion'] = torch.stack(in_data['fut_motion_3D'], dim=0).to(device).transpose(0, 1).contiguous() # dim:[12, agents, 2]
        self.data['fut_motion_orig'] = torch.stack(in_data['fut_motion_3D'], dim=0).to(device)   # future motion without transpose
        self.data['fut_mask'] = torch.stack(in_data['fut_motion_mask'], dim=0).to(device) # dim: [agents,12]
        self.data['pre_mask'] = torch.stack(in_data['pre_motion_mask'], dim=0).to(device) # dim: [agents,8]
        self.data['scene_orig'] = torch.cat([self.data['pre_motion'], self.data['fut_motion']]).view(-1, 2).mean(dim=0) # dim: [2]

        # torch.cat([self.data['pre_motion'], self.data['fut_motion']]) size : [20, agents, 2]
        # .view(-1, 2) size : [ 20 * agents, 2]
        #  Here we bring 20 timesteps "agents" x,y coords to single axis i.e [20*agents, 2] which means it maintains x, y coords of all agents
        #  across 20 timesteps in list of list, now we mean x coord of all agents across all timesteps and same for y coord

        if in_data['heading'] is not None:
            self.data['heading'] = torch.tensor(in_data['heading']).float().to(device)

        # rotate the scene
        # rotate about self.data['scene_orig'] point which is treated as origin
        # normalization is done similar to trajectron++ which is subtracting x,y coords with its respective means
        ...
        self.data['pre_vel'] = self.data['pre_motion'][1:] - self.data['pre_motion'][:-1, :] # dim: [7, agents, 2]
        self.data['fut_vel'] = self.data['fut_motion'] - torch.cat([self.data['pre_motion'][[-1]], self.data['fut_motion'][:-1, :]]) #dim:[12,a,2]

        self.data['cur_motion'] = self.data['pre_motion'][[-1]] # dim: [1, agents, 2]
