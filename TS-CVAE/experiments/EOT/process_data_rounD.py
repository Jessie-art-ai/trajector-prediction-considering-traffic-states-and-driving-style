...
def augment_scene(scene, angle, state_value, style_value):
    def rotate_pc(pc, alpha):
        M = np.array([[np.cos(alpha), -np.sin(alpha)],
                      [np.sin(alpha), np.cos(alpha)]])
        return M @ pc

    data_columns = pd.MultiIndex.from_product([['position', 'velocity', 'acceleration', 'traffic_state', 'driving style'], ['x', 'y']]) #added state
    ...
    for node in scene.nodes:
        ...
        data_dict = {('position', 'x'): x,
                     ...
                     ('traffic_state', 'y'): state_value,
                     ('driving style','x'): style_value,
                     ('driving style','y'): style_value  # added
                    }
...
data_columns = pd.MultiIndex.from_product([['position', 'velocity', 'acceleration', 'traffic_state', 'driving style'], ['x', 'y']]) #added state variable
...
### added ###
train_files, val_files, test_files = [], [], []
for file in os.listdir('raw/rounD_split/create_state_data/train/'):
    train_files.append('raw/rounD_split/create_state_data/train/' + file)

for file in os.listdir('raw/rounD_split/create_state_data/test/'):
    test_files.append('raw/rounD_split/create_state_data/test/' + file)

for file in os.listdir('raw/rounD_split/create_state_data/val/'):
    val_files.append('raw/rounD_split/create_state_data/test/' + file)
### added ###


for desired_source in ['create_state_data']:#['EOT_2fps_10_state']:
    for data_class in ['train', 'val', 'test']:
        env = Environment(node_type_list=['Car'], standardization=standardization)
        attention_radius = dict()
        attention_radius[(env.NodeType.Car, env.NodeType.Car)] = 10.0 # should be different for car class in EOT dataset
        ...
        for _ in range(1):
            ...
            for file in files:
                if file.endswith('.txt'):
                    ...
                    state_value = -1
                    temp_split = (file.split("/")[-1]).split('_')[2]
                    if temp_split == "clump":
                        state_value = 0
                    ...
                    data.columns = ['frame_id', 'track_id', 'pos_x', 'pos_y', 'road_user_type', 'driving style'] #added road user type
                    ...
                    # added #
                    style_value = -1
                    style_value = data['driving style']
