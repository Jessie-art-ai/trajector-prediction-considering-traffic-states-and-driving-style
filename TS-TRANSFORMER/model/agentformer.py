...
class ContextEncoder(nn.Module):
    ...
    def forward(self, data, style_name=None):
        ...
        traj_in = torch.cat(traj_in,
                            dim=-1)  # dim: [8, 1, 4] # similar to trajectron++, we keep pos, velocity for every agent in single list
        ...
        tf_in_pos = self.pos_encoder(tf_in, num_a=data['agent_num'],
                                     agent_enc_shuffle=agent_enc_shuffle)  # dim: [8, 1, 256]

        src_agent_mask = data['agent_mask'].clone()  # dim: [agents, agents]
        src_mask = generate_mask(tf_in.shape[0], tf_in.shape[0], data['agent_num'], src_agent_mask).to(
            tf_in.device)  # dim: [8, 8]
...
class FutureEncoder(nn.Module):
    ...
    def forward(self, data, reparam=True):
        ...
        agent_enc_shuffle = data['agent_enc_shuffle'] if self.agent_enc_shuffle else None
        tf_in_pos = self.pos_encoder(tf_in, num_a=data['agent_num'],
                                     agent_enc_shuffle=agent_enc_shuffle)  # dim: [12, 1, 256]

        mem_agent_mask = data['agent_mask'].clone()  # dim: [agents, agents]
        tgt_agent_mask = data['agent_mask'].clone()

        mem_mask = generate_mask(tf_in.shape[0], data['context_enc'].shape[0], data['agent_num'], mem_agent_mask).to(
            tf_in.device)  # dim: [12, 8]
        tgt_mask = generate_mask(tf_in.shape[0], tf_in.shape[0], data['agent_num'], tgt_agent_mask).to(
            tf_in.device)  # dim: [12, 12]
...
class FutureDecoder(nn.Module):
    ...
    def decode_traj_ar(self, data, mode, context, pre_motion, pre_vel, pre_motion_scene_norm, z, sample_num,
        ...
        for i in range(self.future_frames):
            tf_in = self.input_fc(dec_in_z.view(-1, dec_in_z.shape[-1])).view(dec_in_z.shape[0], -1,
                                                                              self.model_dim)  # dim: [1, 1, 256]
            agent_enc_shuffle = data['agent_enc_shuffle'] if self.agent_enc_shuffle else None
            ...
            if self.out_mlp_dim is not None:
                cat_arr = [out_tmp]
                if 'map' in self.out_cat:
                    map_tmp = data['map_enc'].unsqueeze(0).repeat((i + 1, sample_num, 1)).view(-1,
                                                                                               data['map_enc'].shape[
                                                                                                   -1])
            ...
            seq_out = self.out_fc(out_tmp).view(tf_out.shape[0], -1,
                                                self.forecast_dim)  # dim: [1, 1, 2] --> [time, agents, 2]
...
class AgentFormer(nn.Module):
    ...
    def set_data(self, data):
        ...
        self.data['pre_motion'] = torch.stack(in_data['pre_motion_3D'], dim=0).to(device).transpose(0,
                                                                                                    1).contiguous()  # dim:[8, agents, 2]
        self.data['fut_motion'] = torch.stack(in_data['fut_motion_3D'], dim=0).to(device).transpose(0,
                                                                                                    1).contiguous()  # dim:[12, agents, 2]
        ...
        self.data['scene_orig'] = torch.cat([self.data['pre_motion'], self.data['fut_motion']]).view(-1, 2).mean(
            dim=0)  # dim: [2]
        ...
        self.data['fut_vel'] = self.data['fut_motion'] - torch.cat(
            [self.data['pre_motion'][[-1]], self.data['fut_motion'][:-1, :]])  # dim:[12,a,2]
