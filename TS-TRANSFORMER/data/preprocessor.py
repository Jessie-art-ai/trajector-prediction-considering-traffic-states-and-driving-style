...
class preprocess(object):
    ...
    def __init__(self, data_root, seq_name, parser, log, split='train', phase='training'):
        ...
        if parser.dataset == 'nuscenes_pred':
            ...
            label_path = seq_name
            delimiter = ' '
        elif parser.dataset == 'rounD':  # 加的rounD数据
            #label_path = os.path.join(data_root, 'label/{}/{}.txt'.format(split, seq_name))
            label_path = seq_name
            delimiter = ','
        else:
    ...
    def GetID(self, data):  # 提取给定数据中的ID
        id = []
        for i in range(data.shape[0]):
            id.append(data[i, 1].copy())
        return id

    def TotalFrame(self):  # 返回总帧数
        return self.num_fr

    def PreData(self, frame):  # 获取指定帧号前的数据集，根据frame_skip参数处理
        DataList = []
    ...
    def get_valid_id(self, pre_data, fut_data):  # 确定有效ID，在给定的历史和未来时间窗口中都存在
    ...
    def __call__(self, frame):
        ...
        pre_data = pre_data[
                   :: -1]  # reverse the pre_data as H, H-1, H-2 which is the history data format of AgentFormer
        ...
        data = {
            ...
            'seq': self.seq_name,
            'frame': frame,
            'driving styles': driving_styles  # 添加驾驶风格
        }
